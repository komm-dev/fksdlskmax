...
bool Hooks::Setup( ) {
	...
	if (!DTR::CreateMoveProxy.Create(MEM::GetVFunc(Interfaces::Client, VTABLE::CREATEMOVE), &hkCreateMoveProxy)) // no freeze here
		return false;
	...
	if (!DTR::FrameStageNotify.Create(MEM::GetVFunc(Interfaces::Client, VTABLE::FRAMESTAGENOTIFY), &hkFrameStageNotify)) // no freeze here
		return false;
