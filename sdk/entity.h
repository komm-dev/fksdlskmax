...
public:
	...
	Vector WorldSpaceCenter( ) {
		return MEM::CallVFunc<Vector>( this, 78); // 54 -> check old if new doesnt work
	}
};
...
public:
	...
	CIKContext()
	{
		...
		static auto fnConstructor = reinterpret_cast<void(__thiscall*)(CIKContext*)>(MEM::GetAbsoluteAddress(MEM::FindPattern(CLIENT_DLL, ("E8 ? ? ? ? A1 ? ? ? ? FF 75 18")) + 0x1));
		fnConstructor(this);
	}

	~CIKContext()
	{
		static auto fnDestructor = reinterpret_cast<void(__thiscall*)(CIKContext*)>(MEM::GetAbsoluteAddress(MEM::FindPattern(CLIENT_DLL, ("E8 ? ? ? ? 8B 44 24 14 85 C0 74 0B")) + 0x1));
		fnDestructor(this);
	}
...
public:
	...
	OFFSET( bool, m_bUseNewAnimstate( ), 0x9B14);
	OFFSET( CAnimationLayer*, m_AnimationLayers( ), 0x2990 );
	OFFSET( CCommandContext, m_CmdContext( ), 0x350C); 
	OFFSET( int, m_nSimulationTick( ), 0x2a8u );// search
	OFFSET( unsigned long, g_iModelBoneCounter( ), 0x2690);// havent tested but - 0x10
	OFFSET( int, m_iOcclusionFrame( ), 0xa30u );
	OFFSET( std::uint32_t, m_iOcclusionFlags( ), 0xa28u );
	OFFSET( std::uint8_t, m_iEntClientFlags( ), 0x68u );// search
	OFFSET( CIKContext*, m_pIk( ), 0x2670); // havent tested but - 0x10
	OFFSET( int, m_iLastSetupBonesFrame( ), 0xa68u );
	OFFSET( float, m_flLastSetupBonesTime( ), 0x2924);// - 0x14
	OFFSET( CBoneAccessor, m_BoneAccessor( ), 0x26A4);// - 0x10
	OFFSET( CStudioHdr*, m_pStudioHdr( ), 0x2950);// - 0x14 // search 0x2950 - 0x293cu
	OFFSET( CBoneMergeCache*, m_pBoneMergeCache( ), 0x2910u ); // huh
	...
	OFFSET( int, m_iAccumulatedBoneMask( ), 0x2475 ); // huh
	OFFSET( int, m_iPrevBoneMask( ), 0x2470 );

	OFFSET( CCSGOPlayerAnimState*, m_pAnimState( ), 0x9960); // @ida: client.dll @ [8B 8E ? ? ? ? 85 C9 74 3E + 0x2]
	OFFSET( CUtlVector< matrix3x4_t >, m_CachedBoneData( ), 0x2914); // not sure
	...
	float GetLayerSequenceCycleRate( CAnimationLayer* layer, int32_t sequence ) {
		return MEM::CallVFunc<float>( this, 223, layer, sequence ); // old 217
	}

	void UpdateDispatchLayer( CAnimationLayer* layer, CStudioHdr* hdr, int seq ) {
		return MEM::CallVFunc<void>( this, 247, layer, hdr, seq ); // idk
	}

	void CalculateIKLocks( float simtime ) {
		return MEM::CallVFunc<void>( this, 193, simtime ); // idk
	}
	...
	void StandardBlendingRules( CStudioHdr* hdr, Vector* pos, Quaternion* q, float time, int mask ) {
		...
		return MEM::GetVFunc< StandardBlendingRules_t >( this, 206)( this, hdr, pos, q, time, mask );
	}
	...
	__forceinline void BuildTransformations( CStudioHdr* hdr, Vector* pos, Quaternion* q, const matrix3x4_t& transform, int mask, uint8_t* computed ) {
		...
		return MEM::GetVFunc< BuildTransformations_t >( this, 190)( this, hdr, pos, q, transform, mask, computed );
	}
...
//{
//public:
//	virtual ~CCSWeaponData() = default;
//
//	char* m_consoleName;				// 0x0004
//	char		pad_0008[8];				// 0x0008
//	void* weapon_def;					// 0x0010
//	int               iMaxClip1;							// 0x0014
//	int				  iMaxClip2;							// 0x0018
//	int				  iDefaultclip1;						// 0x001C
//	int		          iDefaultclip2;						// 0x0020
