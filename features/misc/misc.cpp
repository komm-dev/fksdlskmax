...
void CMisc::AutoStop( CUserCmd& cmd ) {
	...
	const auto maxWeaponSpeed{ ( ctx.m_pLocal->m_bIsScoped( ) ? ctx.m_pWeaponData->flMaxSpeed[1] : ctx.m_pWeaponData->flMaxSpeed[0])};
...
//0x224
#include <cmath>
void CMisc::NormalizeMovement( CUserCmd& pCmd, QAngle& oldang) {
	// pCmd.viewAngles.Normalize( );
	//cmd.viewAngles.Clamp( );

	static CConVar* cl_forwardspeed = Interfaces::ConVar->FindVar("cl_forwardspeed");
	static CConVar* cl_sidespeed = Interfaces::ConVar->FindVar("cl_sidespeed");
	...
	QAngle aRealDir = pCmd.viewAngles;
	aRealDir.Normalize();
	aRealDir.Clamp();
	...
	pCmd.flForwardMove = std::clamp(x + (flMoveDelta * pCmd.viewAngles.z), -450.f, 450.f);
	pCmd.flSideMove = std::clamp(y + (flMoveDelta * pCmd.viewAngles.z), -450.f, 450.f);
